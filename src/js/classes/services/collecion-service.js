...
export default class CollectionService extends LocalStorageService {
  ...
  getCollectionIdsBundle({ collection, page = 1 }) {
    ...
    const bundleArray =
      this.#totalPages[collection] === 1
        ? this.#collections[collection]
        : this.#collections[collection].slice(
            startPosition,
            startPosition + this.perPage
          );
    const totalResults = this.#collections[collection].length;
    return {
      bundle: bundleArray,
      totalPages: this.#totalPages[collection],
      page: this.currentPage[collection],
  ...
  #totalPagesCount() {
    const keys = Object.keys(this.#collections);
    keys.forEach(key => {
      this.#totalPages[key] = Math.ceil(
        this.#collections[key].length / this.perPage
      );
    });
  }
}
