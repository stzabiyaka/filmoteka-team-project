export default class CollectionHandler {
  ...
  async getCollectionMoviesPage({ collectionName, page = 1 }) {
    if (
      !this.#collectionsService.isCollectionExist({
    ...
    if (
      !this.#collectionsService.isPageExist({
        collection: collectionName,
        page: page,
      })
    ) {
    ...
    const bundle = this.#collectionsService.getCollectionIdsBundle({
      collection: collectionName,
      page: page,
    });
    const loader = this.#apiService.getMoviesBundle.bind(this.#apiService);
    const response = await this.#markupRender.renderLiblary({
      loader: loader,
      content: bundle,
    });
    return response;
